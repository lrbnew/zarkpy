...
class TestImage(unittest.TestCase):
    ...
    def test_insert_savesuccess(self):
        ...
        data = sh.storage({'data_name': 'User', 'data_id': 1, image_model.image_key: test_image})
        ...
        # 用数据id命名, 并保持相同后缀
        self.assertTrue(image_path.endswith('/%d.%s' % (id, test_image.imagetype)))
        # 保存的文件数据与测试数据一致
    ...
    def test_insert_convert(self):
        ...
        # 依然是原来的尺寸
        self.assertEqual(Image.open(image_path).size, Image.open(test_image.filename).size)
