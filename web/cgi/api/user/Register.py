...
class Register:
    ...
    def POST(self, inputs=None):
        ...
        if error:
            return sh.toJsonp({'success': False, 'error': error})
        ...
        if sh.model('User').validation_request:
            ...
            return sh.toJsonp({'success': True, 'Userid': new_id, 'msg': '注册成功,请查收您的验证邮件'})
        else:
            return sh.toJsonp({'success': True, 'Userid': new_id, 'msg': '注册成功'})
